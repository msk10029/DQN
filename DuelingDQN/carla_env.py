...
class CarlaEnv:
    ...
    def reset(self, episode):
        ...
        if (episode > ENV_CHANGE_EPISODE):
            spawn_points = self.world.get_map().get_spawn_points()
            random_spawn = random.choice(spawn_points)
            self.vehicle = self.world.try_spawn_actor(vehicle_bp, random_spawn)
        ...
        if self.vehicle is None:
            ...
            return self.reset(episode)  # Try again
        # print("Out")
    ...
    def cleanup(self):
        """Destroy vehicle and all attached sensors to avoid memory leaks."""
        if self.vehicle:
            self.vehicle.destroy()
